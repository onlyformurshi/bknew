...
// Get the sponsorship ID from GET
$sponsorship_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
$sponsorship = null;
                                ...
                                <div class="form-row">
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <select class="form-control select2" name="user_id" id="user_id" required>
                                            ...
                                            <?php foreach ($sponsors as $s): ?>
                                                <option value="<?= $s['id'] ?>" <?= (isset($sponsorship['user_id']) && $sponsorship['user_id'] == $s['id']) ? 'selected' : '' ?>>
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <select class="form-control select2" name="program_id" id="program_id" required>
                                            ...
                                            <?php foreach ($programs as $p): ?>
                                                <option value="<?= $p['id'] ?>" <?= (isset($sponsorship['program_id']) && $sponsorship['program_id'] == $p['id']) ? 'selected' : '' ?>>
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <select class="form-control select2" name="category" id="category" required>
                                            ...
                                            <?php foreach ($categories as $key => $cat): ?>
                                                <option value="<?= $key ?>" <?= (isset($sponsorship['category']) && $sponsorship['category'] == $key) ? 'selected' : '' ?>><?= htmlspecialchars($cat) ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <select class="form-control select2" name="item_id" id="item_id" required>
                                            ...
                                            <!-- Items will be loaded by JS, but you can optionally pre-select the current item -->
                                        </select>
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <input type="number" class="form-control" name="amount" id="amount" min="1" required value="<?= $sponsorship['amount'] ?? '' ?>">
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <select class="form-control" name="payment_status" id="payment_status" required>
                                            ...
                                            <?php foreach ($paymentStatuses as $status): ?>
                                                <option value="<?= htmlspecialchars($status) ?>" <?= (isset($sponsorship['payment_status']) && $sponsorship['payment_status'] == $status) ? 'selected' : '' ?>><?= ucfirst(htmlspecialchars($status)) ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <select class="form-control" name="payment_type" id="payment_type" required>
                                            ...
                                            <?php foreach ($paymentTypes as $type): ?>
                                                <option value="<?= htmlspecialchars($type) ?>" <?= (isset($sponsorship['payment_type']) && $sponsorship['payment_type'] == $type) ? 'selected' : '' ?>><?= ucfirst(htmlspecialchars($type)) ?></option>
                                            <?php endforeach; ?>
                                        </select>
...
<script>
    $(document).ready(function() {
        ...
        $('#category, #program_id').on('change', function() {
            loadItems();
        });
        ...
        function loadItems() {
            ...
            if (category && programId) {
                $.getJSON('get-items.php', {
                    ...
                    <?php if (isset($sponsorship['item_id'])): ?>
                    $itemSelect.val('<?= $sponsorship['item_id'] ?>');
                    <?php endif; ?>
        ...
        // On page load, trigger loadItems if editing
        <?php if (isset($sponsorship['category']) && isset($sponsorship['program_id'])): ?>
        $(function() {
            $('#category').val('<?= $sponsorship['category'] ?>');
            $('#program_id').val('<?= $sponsorship['program_id'] ?>');
            loadItems();
        });
        <?php endif; ?>
